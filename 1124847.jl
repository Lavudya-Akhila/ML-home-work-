mutable struct KMeans
    ...
    max_iterations::Int
    tolerance::Float64
    codebook::Vector{Vector{Float64}}
end
...
function update_codebook(kmeans::KMeans, data, clusters)
    ...
    for i in 1:kmeans.k
        ...
        if !isempty(cluster_points)
            push!(new_codebook, [my_mean([point[j] for point in cluster_points]) for j in 1:length(cluster_points[1])])
        else
